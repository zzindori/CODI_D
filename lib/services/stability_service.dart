...
class StabilityService {
  ...
  Future<File?> imageToImage({
    ...
    try {
      ...
      debugPrint(
        '[Stability] Prepared Size: ${prepared.width}x${prepared.height}',
      );
      debugPrint('[Stability] Prepared Bytes: ${prepared.bytes.length} bytes');
      debugPrint('[Stability] Image Strength: $strength');
      debugPrint('[Stability] CFG Scale: $cfgScale');
      debugPrint('[Stability] Steps: $steps');
      ...
      if ((negativePrompt ?? '').trim().isNotEmpty) {
        debugPrint(
          '[Stability] Negative Prompt Length: ${negativePrompt!.length} characters',
        );
      }
      debugPrint(
        '[Stability] Endpoint: $_v1BaseUrl/generation/$_v1ImageToImageEngine/image-to-image',
      );
      ...
      final request =
          http.MultipartRequest(
              'POST',
              Uri.parse(
                '$_v1BaseUrl/generation/$_v1ImageToImageEngine/image-to-image',
              ),
            )
            ..headers['Authorization'] = 'Bearer $_apiKey'
      ...
      if ((negativePrompt ?? '').trim().isNotEmpty) {
        request.fields['text_prompts[1][text]'] = negativePrompt!;
  ...
  ({List<int> bytes, int width, int height}) _prepareImageForSdxlV1(
    ...
    final offsetX = ((resized.width - targetW) / 2).floor().clamp(
      0,
      resized.width - targetW,
    );
    final offsetY = ((resized.height - targetH) / 2).floor().clamp(
      0,
    ...
    return (bytes: img.encodePng(cropped), width: targetW, height: targetH);
  }
  ...
  Future<File?> inpainting({
    ...
    try {
      ...
      final duration = DateTime.now().difference(startTime);
      debugPrint(
        '[Stability] Inpainting 응답 받음 (소요 시간: ${duration.inSeconds}초)',
      );
      debugPrint('[Stability] HTTP Status: ${streamed.statusCode}');
  ...
  Future<File?> evolveAvatarSilhouette({required String bodyType}) async {
  ...
  Future<File?> generateCoordinationImage({
    ...
    try {
      ...
      debugPrint('[StabilityV3] ════════════════════════════════════════');
      debugPrint(
        '[StabilityV3] Prompt Length: ${coordinationPrompt.length} characters',
      );
      debugPrint('[StabilityV3] Aspect Ratio: $aspectRatio');
      debugPrint('[StabilityV3] Seed: ${seed ?? "random"}');
      debugPrint(
        '[StabilityV3] Prompt Preview: ${coordinationPrompt.substring(0, math.min(200, coordinationPrompt.length))}...',
      );
